    ...
    <Modal onClose={() => { resetCsv(); onClose(); }}>
      ...
      {tab === "csv" && (
        <>
          ...
          <div className="csv-steps">
            {["upload", "map", "preview"].map((s, i) => (
              <div key={s} className="flex items-center gap-8">
                ...
                {i < 2 && <span style={{ color: "var(--input-border)", fontSize: 16 }}>›</span>}
              </div>
            ))}
          </div>
          ...
          {csvStep === "upload" && (
            <>
              <div className="card mb-20" style={{ padding: 16 }}>
                ...
                {allLists.length > 0 && (
                  <div className="flex gap-6 flex-wrap mt-8">
                    <span className="mono" style={{ fontSize: 11, color: "var(--text-muted)", alignSelf: "center" }}>Existing:</span>
                ...
                {!csvListName && <div style={{ fontSize: 11, color: "var(--warning-alt)", marginTop: 8 }}>⚠ Recommended — helps you filter and manage this batch later</div>}
              </div>
              ...
              >
                ...
                <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 4 }}>Drop your CSV here or click to browse</div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Supports .csv files · up to 10,000 rows</div>
              ...
              {csvError && <div style={{ background: "var(--danger-bg)", border: "1px solid var(--danger-border)", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "var(--danger)", marginBottom: 16 }}>{csvError}</div>}
              <div className="flex items-center justify-between card" style={{ padding: "12px 16px" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 2 }}>Need a template?</div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Download our CSV template with all supported columns</div>
                </div>
          ...
          {csvStep === "map" && (
            <>
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Map Your Columns</div>
              <div className="flex items-center gap-10 mb-16">
                <div style={{ fontSize: 13, color: "var(--text-sec)" }}>Found <strong style={{ color: "var(--text)" }}>{csvRaw.length} rows</strong>. Match CSV columns to fields.</div>
              ...
              <div className="flex flex-col gap-8 mb-20">
                {CSV_FIELDS.map((f) => (
                  <div key={f.key} className="flex items-center gap-10">
                    <div style={{ width: 130, flexShrink: 0, fontSize: 12, color: f.required ? "#a78bfa" : "var(--text-sec)", fontWeight: f.required ? 600 : 400 }}>
                      {f.label}{f.required && <span style={{ fontSize: 10, color: "var(--primary)", marginLeft: 3 }}>*</span>}
                    </div>
                    ...
                    {csvMapping[f.key] && <div className="mono truncate" style={{ fontSize: 11, color: "var(--text-dim)", width: 90 }}>e.g. {csvRaw[0]?.[csvHeaders.indexOf(csvMapping[f.key])] || "—"}</div>}
                  </div>
                ))}
              </div>
              {(!csvMapping.name || !csvMapping.company) && <div style={{ background: "#2a2a1e", border: "1px solid var(--warning-border)", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "var(--warning)", marginBottom: 14 }}>Name and Company are required.</div>}
          ...
          {csvStep === "preview" && (
            <>
              <div className="flex items-center gap-10 mb-6">
                <div style={{ fontSize: 16, fontWeight: 700 }}>Review & Import</div>
                {csvListName && <span className="filter-pill" style={{ background: "var(--primary-bg)", borderColor: "var(--primary)" }}>📋 {csvListName}</span>}
              </div>
